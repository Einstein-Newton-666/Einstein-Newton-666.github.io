...
文章页有**两个入口**，点了都会打开 `/admin/?p=<文件名>` 并直接载入这篇文章：

- **桌面**：右侧悬浮工具条（与"目录/评论"同一排）里的铅笔按钮。
- **移动端**：标题下方元信息行里的「✎ 编辑」链接。之所以要第二处，是因为主题在 `≤640px`
  会把整个 `.post-tools` 隐藏（`style.css` 里 `@media (max-width:640px){ .page-container .post-tools{display:none} }`），
  悬浮按钮在手机上根本点不到。
...
- 两处入口的样式都**内联在元素上**：加密门只把 `<head>` 里的 `<script>` 搬进密文负载，`<style>`
  会留在明文区，写成 `<style>` 会让规则本身明文可见（等于暴露按钮存在）。
- 移动端入口的显隐**复用主题自带的 `.desktop` / `.mobile` 类**（主题本来就靠这两个类切换日期显示），
  因此不必自己写媒体查询，也就不需要往 head 里塞 `<style>`。
...
源码在私有仓库里，公开仓库只收构建产物（2026-09-16 起生效）。本地 `origin` 指向私有源码仓库，
公开仓库保留为名为 `Einstein-Newton-666.github.io` 的 remote，只由 CI 推送、日常不要往它推。
