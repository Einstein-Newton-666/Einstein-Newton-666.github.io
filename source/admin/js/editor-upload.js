(function (root, factory) {
   ...
   */
  function validateFiles(files, existingNames = []) {
    const accepted = [];
    const rejected = [];
  ...
  function createQueue(options = {}) {
    ...
    function add(files, context = {}) {
      const { accepted, rejected } = validateFiles(Array.from(files || []), existingNames());
      const pushed = [];
      for (const entry of accepted) {
        const name = doc.uniqueAssetName(entry.name, existingNames());
        const item = {
          name,
    ...
    async function toCommitFiles() {
      ...
      for (const item of items) {
        ...
        seen.add(path);
        files.push({ path, content: bytesToBase64(bytes), digest, name: item.name, item });
      }
      return files;
    }
