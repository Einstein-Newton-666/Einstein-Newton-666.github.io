(function (root, factory) {
  ...
  function parseBlockLines(blockText) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      entries.push({ key: match[1], raw: line, value: parseScalar(match[2].trim()) });
    }
    return entries;
  }
  ...
  function serializeDocument(doc, meta, options = {}) {
    ...
    for (const entry of entries) {
      if (entry.key && Object.prototype.hasOwnProperty.call(overrides, entry.key)) {
        ...
        rendered.set(entry.key, `${entry.key}: ${serializeScalar(overrides[entry.key])}`);
        patched += 1;
        continue;
      }
      if (entry.key && rendered.has(entry.key)) continue;
      // 没有新增键时逐行原样保留；有新增键时，在第一个规范位置之前的已知键处让位给新键
      if (hasNewKeys && entry.key && KEY_ORDER.includes(entry.key) && patched === 0) continue;
  ...
  function validatePost({ slug, meta, body, assetFiles = [] } = {}) {
    ...
    if (data.date !== undefined && data.date !== null && String(data.date).trim() !== '' && !DATE_PATTERN.test(String(data.date).trim())) {
