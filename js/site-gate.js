(function (root, factory) {
  ...
  /**
   * 取 storage 对象本身也要包在 try 里。
   *
   * 有些浏览器（禁用了 Cookie/站点数据、被沙箱化的 iframe、部分隐私模式）在**读
   * window.localStorage 这个属性时**就抛 SecurityError，而不是等到 getItem。
   * 以前是把 root.localStorage 当实参传进来，那次属性求值发生在 try 之外 ——
   * 守卫接不住，boot() 末尾的 readCachedKey() 会直接把整个解锁流程打断。
   */
  ...
  function readCachedKey() {
    const stored = storageGet('localStorage', PERSIST_STORE_KEY);
    if (stored) {
      try {
      ...
      storageRemove('localStorage', PERSIST_STORE_KEY);
    }
    return storageGet('sessionStorage', SESSION_STORE_KEY);
  }

  function rememberKey(base64Key, persist) {
    if (persist) {
      storageSet('localStorage', PERSIST_STORE_KEY, JSON.stringify({
        key: base64Key,
        exp: Date.now() + PERSIST_DAYS * 24 * 60 * 60 * 1000,
      }));
      storageRemove('sessionStorage', SESSION_STORE_KEY);
      return;
    }
    storageSet('sessionStorage', SESSION_STORE_KEY, base64Key);
  }
